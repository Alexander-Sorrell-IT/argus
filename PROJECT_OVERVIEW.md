...
|---|---|
| Detection | SPL `eventstats` (z-score per contract), `streamstats`, `predict`, `cluster`, + a mechanism-aware privileged-call rule (`lookup` of admin selectors) — 13 detections + 3 scoring/baseline jobs + 2 ES-ready outputs (RBA, adaptive response) = 18 saved searches; an optional MLTK clustering search ships disabled |
